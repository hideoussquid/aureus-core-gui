...
`test/functional/test_framework/test_framework.py:AureusTestFramework.main`.
...
ZMQ_SCRIPTS = [
    # ZMQ test can only be run if aureus was built with zmq-enabled.
...
def main():
    ...
    enable_aureusd = config["components"].getboolean("ENABLE_AUREUSD")
    ...
    if not (enable_wallet and enable_utils and enable_aureusd):
        print("No functional tests to run. Wallet, utils, and aureusd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, jobs=1, enable_coverage=False, args=[]):
    ...
    if "AUREUSD" not in os.environ:
        os.environ["AUREUSD"] = build_dir + '/src/aureusd' + exeext
